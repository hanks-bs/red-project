...
gulp.task("tscompile", () => {
	return gulp
		.src([
			...
			"!./src/assets/scripts/calc-btw.ts",
		])
...
gulp.task("calculator-btw", () => {
	return gulp
		.src(["./src/assets/scripts/calc-btw.ts"])
		.pipe(
			lazyplugins.typescriptcompiler({
				allowJs: true,
				target: "es2018",
		...
		.pipe(lazyplugins.uglify())
		.pipe(lazyplugins.concat("calculator-btw.min.js"))
		.pipe(
			lazyplugins.uglify({
				mangle: {
					toplevel: true,
...
gulp.task("contactFormCompile", () => {
	return gulp
		...
		.pipe(lazyplugins.concat("emailSend.min.js"))
		.pipe(
			lazyplugins.uglify({
				mangle: {
					toplevel: true,
